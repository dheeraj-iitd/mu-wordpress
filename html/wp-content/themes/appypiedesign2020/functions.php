...
/*insert menu class into <a> tag*/
add_filter( 'nav_menu_link_attributes', function($atts) {
        $atts['class'] = "nav-link";
        return $atts;
}, 100, 1 );

/*include simple-text-shortcode.php file*/
require get_template_directory() . '/inc/simple-text-shortcodes.php';

/*include review-aggregator.php file*/
require get_template_directory() . '/inc/review-aggregator.php';

/*include parmalink.php file*/
require get_template_directory() . '/inc/parmalink.php';

/*include Organization-schema.php file*/
require get_template_directory() . '/inc/Organization-schema.php';

/*include geoip.inc file*/
require get_template_directory() . '/inc/geoip.inc';

/*include sharpspring-traking.php file*/
require get_template_directory() . '/inc/sharpspring-traking.php';

/*include CountryIP.php file*/
require get_template_directory() . '/inc/CountryIP.php';

/*include custome-link.php file*/
//require get_template_directory() . '/inc/custome-link.php';

// Remove strip_tags fuction
...
function prefix_nav_description( $item_output, $item, $depth, $args ) {
    if ( !empty( $item->description ) ) {
        $item_output = str_replace( $args->link_after . '</a>', $args->link_after . '<p class="menu-item-description">' . $item->description . '</p>' . '</a>', $item_output );
    }
    return $item_output;
}
...
//add schema atribut SiteNavigationElement by surendra
...
function add_attribute( $items, $args ) {
        $dom = new DOMDocument();
		$dom->loadHTML('<meta http-equiv="Content-Type" content="text/html; charset=utf-8">' .$items);
        $find = $dom->getElementsByTagName('span');
		//$find1 = $dom->getElementsByTagName('ul');
        foreach ($find as $item ) :
            $item->setAttribute('itemprop','name');
        endforeach;

		/* foreach ($find1 as $item ) :
            $item->setAttribute('itemscope','itemscope');
			$item->setAttribute('itemtype','https://schema.org/SiteNavigationElement');
        endforeach;
         */
         return preg_replace('/^<!DOCTYPE.+?>/', '', str_replace( array('<html>', '</html>', '<head>', '</head>','<meta http-equiv="Content-Type" content="text/html; charset=utf-8">','<body>', '</body>'), array('', '', '', '','','',''), $dom->saveHTML()));
